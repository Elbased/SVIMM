...
uniform int useTex;
uniform vec3 biomeFogColor;
uniform vec3 colorTint; // multiplied with base color (default 1,1,1)
...
const vec3 baseFogColor = vec3(0.55, 0.65, 0.85);
...
void main() {
    ...
    vec3 tint = colorTint;
    if (tint.r < 0.01 && tint.g < 0.01 && tint.b < 0.01) tint = vec3(1.0);
    vec3 baseColor = fragColor * tint;
    if (useTex == 1) {
        vec4 texColor = texture(tex, fragUV);
        baseColor = texColor.rgb * tint;
    }
    ...
    // distance fog — blend base and biome fog colors
    vec3 fogColor = mix(baseFogColor, biomeFogColor, 0.6);
